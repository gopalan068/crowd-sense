...
export default function AuditLogView({ logs = [], playbookSteps = [], assistantInstructions = [], onRefresh }) {
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-bold text-sm tracking-wide uppercase" style={{ color: 'var(--color-text)' }}>
            Read-Only Incident, Playbook &amp; Assistant Audit Trail
          </h2>
        </div>

        {/* Filter Pills */}
        <div className="flex items-center gap-2 text-xs font-mono-num flex-wrap">
          {['ALL', 'PANIC', 'ESCALATED', 'ACKNOWLEDGED', 'ASSISTANT GUIDANCE', 'PLAYBOOK STEPS'].map((mode) => (
            <button
              key={mode}
          ...
          <table className="w-full text-left border-collapse text-xs font-mono-num">
            <thead className="sticky top-0 z-10 shadow-xs" style={{ background: 'var(--color-surface)' }}>
              ...
              >
                ...
                <th className="py-2.5 px-4">Automated Action Instruction</th>
            ...
            <tbody className="divide-y" style={{ borderColor: 'var(--color-border)' }}>
              {assistantInstructions.length === 0 ? (
                <tr>
                  <td colSpan={7} className="text-center py-6" style={{ color: 'var(--color-muted)' }}>
                    No automated assistant guidance instructions recorded in audit log yet.
                  </td>
                </tr>
              ) : (
                assistantInstructions.map((inst) => (
                  <tr
                    key={inst.instruction_id || inst.instructionId}
                  ...
                  >
                    <td className="py-2.5 px-4 font-bold text-sky-600 dark:text-sky-400">
                      {inst.instruction_id || inst.instructionId}
                    </td>
                    <td className="py-2.5 px-4 font-bold">{inst.zone_id || inst.zoneId}</td>
                    <td className="py-2.5 px-4 font-bold uppercase text-slate-600 dark:text-slate-300">
                      {inst.event_type || inst.eventType}
                    </td>
                    <td className="py-2.5 px-4">
                      <span
                        ...
                        style={{
                          background:
                            (inst.severity || '').toLowerCase() === 'red' ? '#DC2626' :
                            (inst.severity || '').toLowerCase() === 'orange' ? '#EA580C' :
                            '#CA8A04',
                        }}
                      >
                        {(inst.severity || 'yellow').toUpperCase()}
                      </span>
                    </td>
                    <td className="py-2.5 px-4 font-sans font-medium text-slate-900 dark:text-slate-100 max-w-lg">
                      {inst.text}
                    </td>
                    <td className="py-2.5 px-4 text-[10px] font-mono-num text-slate-500">
                      {inst.source === 'groq_llm' ? '⚡ Groq LLM' : '🔒 Deterministic'}
                    </td>
                    <td className="py-2.5 px-4 font-bold text-slate-600 dark:text-slate-400">
                      {new Date(inst.generated_at || inst.timestamp).toLocaleTimeString()}
                    </td>
                  </tr>
                ))
              )}
