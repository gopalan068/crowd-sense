...
```json
{
  ...
  "area_sqm": 20,
  "density": 2.1,
  "density_source": "detection | override_live | override_cached",
  "saturated": false,
...
| `people_count`    | integer         | CV service | Detection count or equivalent count from calibrated override          |
| `area_sqm`        | float           | CV config  | Manually configured constant — no homography correction               |
| `density`         | float           | CV service | Raw detected density or calibrated override fallback (people/m²)      |
| `density_source`  | `"detection"` \| `"override_live"` \| `"override_cached"` | CV service | Provenance of density estimate (detection vs saturation fallback) |
| `saturated`       | boolean         | CV service | True when crowd saturation threshold is crossed                       |
...
| `density_source`| `"detection"` \| `"override_live"` \| `"override_cached"` | Backend | Passed through from CV payload                     |
| `saturated`     | boolean                                   | Backend | Passed through from CV payload                               |
